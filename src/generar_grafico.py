...
# (color de zona a saturacion plena, nombre, descripcion breve)
ZONE_INFO = {
    ("alta_urg", "alta_fac"):  ("#A63A2E", "Intervenir ya",
        "Urgente y con buen acceso: desplegar de inmediato."),
    ("alta_urg", "media_fac"): ("#C9822E", "Intervenir con gestión de riesgo",
        "Urgente con dificultades moderadas: actuar con planificación."),
    ("alta_urg", "baja_fac"):  ("#8C2F26", "Resolver acceso primero",
        "Urgente pero con acceso bloqueado: gestionar la vía antes de desplegar."),
    ("baja_urg", "alta_fac"):  ("#3F7D6B", "Oportunidad",
        "No urgente y de fácil acceso: atender cuando haya capacidad libre."),
    ("baja_urg", "media_fac"): ("#6E8A9E", "Programar con preparación",
        "No urgente, con dificultades moderadas: programar con anticipación."),
    ("baja_urg", "baja_fac"):  ("#8A8D89", "Monitorear",
...
for i in range(len(urg_bounds) - 1):
    ...
    for j in range(len(fac_bounds) - 1):
        ...
        base_color, zona_nombre, _ = ZONE_INFO.get((urg_key, fac_key), ("#DDDDDD", "", ""))
        color = tint(base_color, 0.90)
        rx, ry = sx(x0), sy(y1)
        ...
        if rw > 55 and rh > 22:
            labels_svg.append(
                ...
                f'font-weight="600" font-family="Open Sans, sans-serif">{zona_nombre}</text>'
            )
...
legend_rows = []
for (u, f), (color, nombre, desc) in ZONE_INFO.items():
    legend_rows.append(
